...
export default function Card({
  ...
  // 根据深色模式调整icon颜色
  const themedIcon = icon && React.isValidElement(icon) ? React.cloneElement(icon as React.ReactElement<any>, {
  ...
  return (
    <View className={`${baseClasses} ${variantClasses[variant]} ${className}`}>
      {(title || icon) && (
        <View className="flex-row items-center px-4 py-2.5 border-b border-gray-200 dark:border-gray-700">
          {themedIcon}
          {title && (
