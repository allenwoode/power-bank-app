...
export default function AddDevicePage() {
	...
	const startScanning = async () => {
		try {
			...
			const errorMessage =
				err instanceof Error ? err.message : t('add-device-scan-failed');
		...
		<>
			...
			<View className="flex-1 bg-gray-100 py-4 dark:bg-black">
				...
				<View className="flex-1">
					...
					<Text className="mb-8 px-6 text-center text-base text-gray-600 dark:text-gray-400">
						{isScanning
					...
					{/* 扫描到设备的列表 - 仅在扫描到设备时显示 */}
					{devices.length > 0 && (
						<View className="mb-3 w-full px-4">
								...
								renderItem={({ item }) => {
									...
									return (
										...
										>
											...
											{/* RSSI 值显示 */}
											<View className="rounded-lg bg-gray-100 px-2 py-1.5 dark:bg-black/40">
												<Text
