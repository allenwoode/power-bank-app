...
import { ChevronLeft, Eye, EyeOff, Lock, Smartphone } from "lucide-react-native";
      ...
      <View className="flex-1 bg-gray-50" style={{ paddingTop: insets.top, paddingBottom: insets.bottom }}>
        ...
        <View className="flex-1 px-8 pt-10">
          <View className="mb-10">
            <Text className="text-3xl font-bold text-gray-900 mb-3">欢迎回来 👋</Text>
          ...
          <View className="space-y-4">
            <View className="flex-row items-center bg-white rounded-2xl px-4 h-14 border border-gray-100 shadow-sm">
              ...
              <TextInput className="flex-1 ml-3 text-base" placeholder="请输入手机号码" value={phone} onChangeText={setPhone} keyboardType="phone-pad" />
            </View>
            <View className="flex-row items-center bg-white rounded-2xl px-4 h-14 border border-gray-100 shadow-sm mt-4">
              <Lock size={20} color="#9CA3AF" />
              <TextInput className="flex-1 ml-3 text-base" placeholder="请输入登录密码" value={password} onChangeText={setPassword} secureTextEntry={!showPassword} />
              <TouchableOpacity onPress={() => setShowPassword(!showPassword)}>
                {showPassword ? <EyeOff size={20} color="#9CA3AF" /> : <Eye size={20} color="#9CA3AF" />}
              </TouchableOpacity>
            </View>
            <TouchableOpacity onPress={handleLogin} className="bg-black h-14 rounded-2xl items-center justify-center mt-6">
              {isLoading ? <ActivityIndicator color="white" /> : <Text className="text-white text-lg font-bold">登录</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </View>
